<template>
  <div>
    <el-menu v-if="studentSidebar" class="sidebar" default-active="/student/Home" router> <!--default-active高亮-->
      <el-menu-item index="/student/Home">  <!--index链接？-->
        <i class="el-icon-search"/>   <!--图片-->
        查看职位
      </el-menu-item>
      <el-menu-item index="/student/candidate/Manage">
        <i class="el-icon-tickets"/>  <!--图片-->
        求职信息管理
      </el-menu-item>
      <el-menu-item index="/student/UploadCV">
        <i class="el-icon-upload2"/>  <!--图片-->
        上传简历
      </el-menu-item>
      <el-submenu index="1">
        <template slot="title">
          <i class="el-icon-setting"/>  <!--图片-->
          个人设置
          <el-badge :is-dot="isPerfect"/> <!--看不出区别-->
        </template>
